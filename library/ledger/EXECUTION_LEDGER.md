# Execution Ledger: PRD-001 thehive Portal Daemon (the-smoker run)

> Category: Ledger | Version: 1.0 | Date: July 2026 | Status: Active

Single source of truth for the `/the-smoker` completion run over **PRD-001 thehive Portal Daemon** (index + 001a/b/c/d). Branch: `feature/prd-001-thehive-portal-daemon`. Status legend: OPEN / IN PROGRESS / DONE / VERIFIED / BLOCKED.

Greenfield implementation in `the-hive` (bare repo). Honeycomb cutover (retire `web/` subtree) runs in Wave 5 after thehive serves.
...
|---|---|---|---|---|
| m-AC-1 | index | thehive runs as its own OS process with `/health`, PID/lock, port 3853 | typescript-node-worker-bee | VERIFIED |
...
| m-AC-5 | index | Independent release train (thehive-only deploy surface) | typescript-node-worker-bee | OPEN (architectural separation confirmed; no CI/CD automation exists yet — see QA report) |
| m-AC-6 | index | honeycomb `/` + `web/` retired only after thehive serves | typescript-node-worker-bee | VERIFIED |
| m-AC-7 | index | hivedoctor supervises via idempotent registry entry, no hivedoctor restart | typescript-node-worker-bee | VERIFIED |
...
| a-AC-4 | 001a | Writes `~/.honeycomb/thehive.pid` and `thehive.lock` | typescript-node-worker-bee | VERIFIED |
...
| a-AC-7 | 001a | App construction pure; only `startThehive` calls `listen()` | typescript-node-worker-bee | VERIFIED |
...
| b-AC-3 | 001b | honeycomb removes `/` mount only after thehive serves | typescript-node-worker-bee | VERIFIED |
...
| c-AC-2 | 001c | hivenectar `/api/source-graph/*` routed to hivenectar | typescript-node-worker-bee | VERIFIED |
...
| c-AC-5 | 001c | No Deep Lake client in thehive | typescript-node-worker-bee | VERIFIED |
| d-AC-1 | 001d | `install-service` writes platform unit (launchd/systemd/schtasks) | typescript-node-worker-bee | VERIFIED |
| d-AC-2 | 001d | Service starts thehive on boot/login | typescript-node-worker-bee | VERIFIED |
| d-AC-3 | 001d | Service restarts thehive on crash | typescript-node-worker-bee | VERIFIED |
| d-AC-4 | 001d | `uninstall-service` removes only thehive unit | typescript-node-worker-bee | VERIFIED |
| d-AC-5 | 001d | thehive unit execs thehive entrypoint, not hivedoctor | typescript-node-worker-bee | VERIFIED |
| d-AC-6 | 001d | Registry contains thehive entry with name, healthUrl, pidPath | typescript-node-worker-bee | VERIFIED |
| d-AC-7 | 001d | Re-install updates entry in place (idempotent) | typescript-node-worker-bee | VERIFIED |
| d-AC-8 | 001d | Atomic JSON write; no hivedoctor restart or code change | typescript-node-worker-bee | VERIFIED |
...
|---|---|---|---|---|
| 1 | `typescript-node-worker-bee` | `gpt-5.3-codex-high-fast` | Greenfield `package.json`, tsconfig, vitest, Hono server, lock, `/health`, `createThehive`/`startThehive`, minimal dashboard host stub | a-AC-* DONE; `npm run typecheck && npm test` green |
...
| 4 | `typescript-node-worker-bee` | `gpt-5.3-codex-high-fast` | Service install/uninstall mirroring hivedoctor; registry RMW | d-AC-* DONE |
| 5 | `typescript-node-worker-bee` | `composer-2.5-fast` | honeycomb: remove `/` mount + delete `web/` after thehive verified | b-AC-3, b-AC-4, m-AC-6 DONE |
...
- Primary code owner: `the-hive/` repository root (not apiary umbrella docs except ledger/PRD lifecycle).
- Reference only (do not import at runtime): `../honeycomb/`, `../hivedoctor/`.
- Honeycomb retirement (Wave 5) touches `honeycomb/` submodule; only after thehive serves dashboard on :3853.
- Do not modify hivenectar or hivedoctor source except honeycomb cutover in Wave 5.

---

## Run log

- Phase 0: PRD moved `backlog/` -> `in-work/`; branch `feature/prd-001-thehive-portal-daemon` created; ledger initialized (29 OPEN).
- Wave 1: DONE (greenfield scaffold and PRD-001a process/bootstrap implemented in `the-hive/src` and `the-hive/tests`).
...
- Wave 2: DONE (PRD-001b dashboard migration — b-AC-1, b-AC-2, b-AC-5 DONE; m-AC-3 IN PROGRESS/partial).
  - Copied the 28 honeycomb `web/` files VERBATIM into `the-hive/src/dashboard/web/` (12 shell/infra + 12 pages + wire.ts/app.tsx/main.tsx/setup-gate.tsx). For Wave 2 the four "copy with modification" files needed no code change: the dashboard pages are origin-agnostic and the asset-base/path adaptation is localized to the host shell (`data-asset-base=""`) and `web-assets.ts` (font prefix `/fonts/`). wire.ts keeps honeycomb's same-origin `/api/*` endpoint map + zod schemas; Wave 3 (PRD-001c) federates it.
  - Copied the PARTIAL `contracts.ts` (only the web-consumed ROI view-model types wire.ts + roi pages import) to `the-hive/src/dashboard/contracts.ts`.
  - Import fix: two verbatim pages import shared modules outside the web tree (`settings.tsx` → `../../../shared/lifecycle-flags.js`, `memories.tsx` → `../../../shared/memory-types.js`). Copied both pure/browser-safe modules to `the-hive/src/shared/` so the relative imports resolve and the pages stay byte-verbatim (no page edit).
  - Copied `assets/styles.css`, `assets/tokens/*`, `assets/logos/honeycomb-memory-cluster.svg` into `the-hive/assets/`.
...
- Wave 3: DONE (PRD-001c federated wire and hivedoctor registry routing implemented).
  - Added `src/shared/daemon-routing.ts` for the static endpoint owner map: honeycomb owns copied dashboard endpoints by default, while `/api/source-graph/*` routes to hivenectar.
  - Added `src/daemon/registry.ts` to read `~/.honeycomb/hivedoctor.daemons.json`, validate entries with zod, derive daemon bases from `healthUrl`, and fall back to documented loopback defaults when the registry is missing or malformed.
  ...
  - Modified `src/dashboard/web/wire.ts` with the daemon-bases architecture: fetch bases once from thehive, wrap fetch so each endpoint is sent to the owning daemon base, and keep existing zod parse/fail-soft return values. The SSE log stream now resolves its honeycomb URL asynchronously before constructing `EventSource`.
- Wave 3 verification evidence:
  ...
  - Deep Lake import audit: `rg` found no `deeplake` imports or requires under `the-hive/src`.
...
- Wave 4: DONE (PRD-001d service-unit install/uninstall and hivedoctor registry registration implemented).
  - Added `src/service/` module (`platform.ts`, `templates.ts`, `commands.ts`, `index.ts`) mirroring hivedoctor's resolve/plan/render/command flow with thehive-specific constants (`label: thehive`, `thehive.service`, Windows task `thehive`) and exec target `node <thehive-cli> start`.
  - Added `src/install/registry.ts` read-modify-write registration for `~/.honeycomb/hivedoctor.daemons.json` with idempotent upsert by `name: "thehive"` and atomic temp-file plus rename writes.
  - Extended `src/cli.ts` command surface: `start`, `install-service`, `uninstall-service`, `register`; `install-service` now installs the platform unit and registers thehive in the hivedoctor registry.
...
- Wave 5: DONE (honeycomb dashboard web portal retirement / ADR-0001 cutover).
  ...
  - Kept honeycomb `/health`, all `/api/*` route groups, ViewBlock/TUI layer (`dashboard.ts`, `views.ts`, `html.ts`, `launch.ts`, `logs.ts`, `contracts.ts`), and setup routes for thehive wire client.
  - Updated `honeycomb install` / `openDashboard` portal URLs to thehive `:3853/`.
...
- Close-out A: DONE (`security-worker-bee` audit of PRD-001 — `the-hive` primary + honeycomb cutover delta).
  ...
  - Report: `the-hive/library/requirements/in-work/prd-001-thehive-portal-daemon/qa/security-report.md`.
...
- Close-out B: DONE (`quality-worker-bee` audit of PRD-001 against the index + 001a/b/c/d ACs).
  - Verified 26 of 27 sub-PRD/module ACs PASS with direct code + test evidence; `npm run typecheck && npm test && npm run build` green in `the-hive` (12 files / 41 tests), plus `honeycomb` typecheck + the 4 cutover-relevant suites (68 tests) green after the Wave 5 delta.
