...
> **Related:** [`ADR-0002`](./ADR-0002-server-side-bff-proxy-for-dashboard-federation.md), [`prd-001c-api-aggregation-wire.md`](../../../requirements/in-work/prd-001-hive-portal-daemon/prd-001c-api-aggregation-wire.md)

## Context

[`ADR-0002`](./ADR-0002-server-side-bff-proxy-for-dashboard-federation.md) established that hive federates dashboard data server-side through a proxy, and that data freshness is delivered by **polling**: the copied dashboard pages hydrate on an interval through `usePoll`, each poll a same-origin `GET` that hive proxies to the owning workload daemon.
...
hive already carries one streaming path: the Logs page tails `/api/logs/stream` via `EventSource`, which the proxy forwards to the owning daemon's SSE endpoint. That proves the proxy can carry a long-lived server-sent-events connection end to end.
...
When real-time freshness is worth the added moving parts, move dashboard data delivery from interval polling to **server-sent events (SSE)** proxied through hive:

- Each workload daemon exposes an SSE endpoint per live view-model (mirroring honeycomb's existing `/api/logs/stream`).
- The dashboard pages subscribe via `EventSource` to same-origin hive routes; hive's proxy forwards the long-lived connection to the owning daemon over loopback (the proxy already streams response bodies, so a `text/event-stream` response is carried through unchanged).
...
- hive must proxy long-lived connections with explicit reconnect and fail-soft handling for every streamed view, more code and tests than a stateless poll.
...
- `hive/src/dashboard/web/wire.ts` - `logsStream` (the existing `EventSource` tail that proves the SSE-through-proxy path) and the `usePoll`-backed reads this would generalize from.
- `hive/src/daemon/proxy.ts` - the proxy that already streams response bodies and would carry the `text/event-stream` connections.
