...
> **MIRROR - not the authoritative copy.** This is a synced copy of the superproject (the-apiary) fleet-wide ADR, placed in hive so the decision is visible when working inside this submodule. Authoritative source: `library/knowledge/private/architecture/ADR-0003-fleet-directory-ownership-and-neutral-state-root.md` in the superproject. Edit the authoritative copy, then re-sync this mirror; do not edit this copy in isolation. (Numbered `ADR-0005` here to fit hive's local sequence; it is superproject `ADR-0003`.)
