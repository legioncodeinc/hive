...
> **Priority:** P1
> **Effort:** S (< 1d)
> **Schema changes:** None. Serves the derived snapshot from 015a over loopback.

---

## Overview

A thin, read-only HTTP surface that hands the 015a `HealthSnapshot` (and per-signal evidence) to the dashboard, mounted beside the existing dashboard host group under the same loopback + local-mode gate. No new bind, no token, no outbound calls. This sub-PRD exists to keep the engine (015a) and the page (015c) decoupled across the daemon/UI seam the repo already uses.
...
- Computing the signals (015a) or rendering them (015c).
...
- Mount under the existing dashboard host group next to [`mountDashboardHost`](../../../../../honeycomb/src/daemon/runtime/dashboard/host.ts), reusing the same local-mode gate and route-registration pattern as [`setup-state.ts`](../../../../../honeycomb/src/daemon/runtime/dashboard/setup-state.ts) and [`sync-api.ts`](../../../../../honeycomb/src/daemon/runtime/dashboard/sync-api.ts).
...
- Keep handlers dumb: they call the 015a engine/cache accessor and serialize. No business logic in the route.
...
- [ ] Shape alignment with the existing dashboard data-fetch conventions ([`wire.ts`](../../../../../honeycomb/src/dashboard/web/wire.ts)).

## Related

- [`src/daemon/runtime/dashboard/host.ts`](../../../../../honeycomb/src/daemon/runtime/dashboard/host.ts) — the host group + local-mode gate.
- [`src/daemon/runtime/dashboard/setup-state.ts`](../../../../../honeycomb/src/daemon/runtime/dashboard/setup-state.ts) and [`sync-api.ts`](../../../../../honeycomb/src/daemon/runtime/dashboard/sync-api.ts) — sibling read-only loopback endpoints to mirror.
- [Request Lifecycle](../../../../../honeycomb/library/knowledge/private/architecture/request-lifecycle.md) — assembly order + gate.
- Sibling sub-PRDs: [015a signal engine](./prd-015a-repository-health-and-knowledge-drift-signal-engine.md), [015c dashboard page](./prd-015c-repository-health-and-knowledge-drift-dashboard-page.md).
