# PRD-015: Repository Health and Knowledge Drift

> **Provenance:** Moved from honeycomb on 2026-07-12 when dashboard/onboarding and repository-workflow surfaces consolidated under Hive (see hive ADR-0001). Renumbered honeycomb PRD-051 → hive PRD-015. Daemon data-plane code paths remain in honeycomb; Hive owns the portal UI and product PRD.
...
- A **read-only Repository Health page** in the dashboard, registered through the existing page registry ([`registry.tsx`](../../../../../honeycomb/src/dashboard/web/registry.tsx) / [`router.tsx`](../../../../../honeycomb/src/dashboard/web/router.tsx) / [`sidebar.tsx`](../../../../../honeycomb/src/dashboard/web/sidebar.tsx)), scoped to the project in the top-left switcher and showing a clear empty state when no project is selected ([`needs-project.tsx`](../../../../../honeycomb/src/dashboard/web/needs-project.tsx)).
...
- A **skill-freshness signal**: skillify watermarks ([`watermark.ts`](../../../../../honeycomb/src/daemon/runtime/skillify/watermark.ts)) lagging meaningfully behind recent session volume, indicating sessions that were never mined into skills.
...
- **Any write to the user's repository.** No doc edits, no scaffold, no Stinger invocation, no auto-fix. This module is read-only by contract. The scaffold is [PRD-016](../prd-016-join-repository-to-hive/prd-016-join-repository-to-hive-index.md); the nudges are [PRD-053](../../../../../honeycomb/library/requirements/backlog/prd-053-coaching-and-reminder-loop/prd-053-coaching-and-reminder-loop-index.md).
- **A hosted or remote dashboard.** This stays a loopback, local-mode-only surface served by the one daemon, exactly like every other dashboard page ([`host.ts`](../../../../../honeycomb/src/daemon/runtime/dashboard/host.ts)). No new server, no new bind, no new auth surface.
...
- **Cross-repository / fleet rollups.** Health is computed for the single selected project. The multi-repo fleet view is a separate thread (see [`fleet-observation-and-on-demand-skills.md`](../../../../../honeycomb/library/knowledge/private/collaboration/asset-sync-substrate.md) and the active fleet-observation design).
...
|---|---|---|
| [`prd-015a-…-drift-and-staleness-signal-engine`](./prd-015a-repository-health-and-knowledge-drift-signal-engine.md) | The daemon-side engine that computes the four signals (knowledge drift, doc staleness, PRD-to-knowledge gap, skill freshness) by joining the codebase graph, the knowledge-doc set, the PRD/IRD lifecycle state, and the skillify watermarks. Produces an explainable, fail-soft per-project health snapshot. | Draft |
| [`prd-015b-…-read-only-health-api`](./prd-015b-repository-health-and-knowledge-drift-read-only-health-api.md) | The loopback, local-mode-only read endpoints that serve the health snapshot and per-signal evidence to the dashboard, beside the existing dashboard host group. Read-only, no-secret, scope-aware. | Draft |
| [`prd-015c-…-repository-health-dashboard-page`](./prd-015c-repository-health-and-knowledge-drift-dashboard-page.md) | The read-only Repository Health page: the rolled-up health band, the per-signal cards, the expand-to-evidence interaction, the no-project and not-enough-data empty states, scoped to the top-left switcher. | Draft |
...
- **Inputs (read-only):** the tree-sitter codebase graph, its extracted **symbol roster**, and content hashes ([`src/daemon/runtime/codebase/extract.ts`](../../../../../honeycomb/src/daemon/runtime/codebase/extract.ts), [`hash.ts`](../../../../../honeycomb/src/daemon/runtime/codebase/hash.ts), [`snapshot.ts`](../../../../../honeycomb/src/daemon/runtime/codebase/snapshot.ts), [`query.ts`](../../../../../honeycomb/src/daemon/runtime/codebase/query.ts)); the `library/knowledge/` doc set on disk (paths, last-modified, and the code references inside each doc); the PRD/IRD lifecycle state derived from `library/requirements/` and `library/issues/` folder placement; the skillify watermarks ([`watermark.ts`](../../../../../honeycomb/src/daemon/runtime/skillify/watermark.ts)) and recent `sessions` volume.
...
All new surface is **loopback + local-mode-only**, mounted beside [`mountDashboardHost`](../../../../../honeycomb/src/daemon/runtime/dashboard/host.ts) under the same gate:
...
- **Evidence-row caps (015b), card ordering and top-N (015c), and standalone-page vs dashboard-home-section (015c)** are implementation details left to `/the-smoker`'s judgement (lean: cap ~20 rows with "N more"; standalone sidebar page for discoverability). Lock any of these only if a later decision requires it.

---

## Related

- [PRD-049: Multi-Project and Context Switching](../../../../../honeycomb/library/requirements/completed/prd-049-multi-project-and-context-switching/prd-049-multi-project-and-context-switching-index.md) and [`scope-context.tsx`](../../../../../honeycomb/src/dashboard/web/scope-context.tsx) — the top-left project switcher this page scopes to.
- [PRD-014: Codebase Graph](../../../../../honeycomb/library/requirements/completed/prd-014-codebase-graph/prd-014-codebase-graph-index.md) and [`src/daemon/runtime/codebase/`](../../../../../honeycomb/src/daemon/runtime/codebase/index.ts) — the tree-sitter graph + content hashes that make structural drift computable.
- [PRD-016: Skillify](../../../../../honeycomb/library/requirements/completed/prd-016-skillify/prd-016-skillify-index.md) and [`src/daemon/runtime/skillify/watermark.ts`](../../../../../honeycomb/src/daemon/runtime/skillify/watermark.ts) — the watermark the skill-freshness signal reads.
- [PRD-017: Wiki Summaries](../../../../../honeycomb/library/requirements/completed/prd-017-wiki-summaries/prd-017-wiki-summaries-index.md) — prior art for deriving doc-shaped artifacts from memory.
- [PRD-024: Dashboard UI Parity](../../../../../honeycomb/library/requirements/completed/prd-024-dashboard-ui-parity/prd-024-dashboard-ui-parity-index.md) and [PRD-037: Dashboard Nav Shell](../../../../../honeycomb/library/requirements/completed/prd-037-dashboard-nav-shell/prd-037-dashboard-nav-shell-index.md) — the self-hydrating, token-free dashboard shell + page registry this page joins.
- [Adding a Page](../../../../../honeycomb/library/knowledge/private/dashboard/adding-a-page.md) — the dashboard wiring this page follows.
- [Daemon Surface](../../../../../honeycomb/library/knowledge/private/architecture/daemon-surface.md) and [Request Lifecycle](../../../../../honeycomb/library/knowledge/private/architecture/request-lifecycle.md) — the local-mode gate and assembly order the health endpoints obey.
- Successor modules: [PRD-016: Join Repository to Hive](../prd-016-join-repository-to-hive/prd-016-join-repository-to-hive-index.md) (the non-destructive scaffold) and [PRD-053: Coaching and Reminder Loop](../../../../../honeycomb/library/requirements/backlog/prd-053-coaching-and-reminder-loop/prd-053-coaching-and-reminder-loop-index.md) (the nudges) both consume these signals.
