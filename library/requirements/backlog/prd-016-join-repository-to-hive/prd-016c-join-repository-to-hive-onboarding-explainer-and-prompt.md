# PRD-016c: Onboarding Explainer and Copy-Paste Prompt
...
- Generate a concise **onboarding explainer** that covers: what each `library/` folder is for; what the starter command(s) provisioned in 016b do; and *why* `/library-stinger` (plan work) and `/knowledge-stinger` (capture knowledge, clear drift) exist and when to reach for each.
...
- Scaffolding (016a) or asset install (016b).
...
- Generate the explainer from a template that interpolates the detected harness + the 016b starter allow-list, so it always matches what was actually installed.
- The dashboard panel can reuse existing primitives/panels ([`panels.tsx`](../../../../../honeycomb/src/dashboard/web/panels.tsx), [`primitives.tsx`](../../../../../honeycomb/src/dashboard/web/primitives.tsx)); this is content, not new UI machinery.
...
- [PRD-016 index](./prd-016-join-repository-to-hive-index.md) — parent flow and write-confinement contract.
- [`src/dashboard/web/panels.tsx`](../../../../../honeycomb/src/dashboard/web/panels.tsx) and [`primitives.tsx`](../../../../../honeycomb/src/dashboard/web/primitives.tsx) — the UI primitives the panel reuses.
- [Library guide / knowledge authoring conventions](../../../../../honeycomb/library/knowledge/private/standards/) — the conventions the explainer summarizes for the user.
- Sibling sub-PRDs: [016a non-destructive library scaffold](./prd-016a-join-repository-to-hive-non-destructive-library-scaffold.md), [016b harness asset provisioning](./prd-016b-join-repository-to-hive-harness-asset-provisioning.md).
- Successor: [PRD-053: Coaching and Reminder Loop](../../../../../honeycomb/library/requirements/backlog/prd-053-coaching-and-reminder-loop/prd-053-coaching-and-reminder-loop-index.md) — turns this one-time prompt into an ongoing nudge.
