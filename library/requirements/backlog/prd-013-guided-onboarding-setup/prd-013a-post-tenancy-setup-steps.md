...
- **Superseded framing (fleet PRD-008):** the multi-provider selector described here is reshaped by apiary [`PRD-008c`](../../../../../library/requirements/backlog/prd-008-unified-fleet-inference-credential/prd-008c-unified-fleet-inference-credential-single-key-dashboard-and-onboarding.md) into a **single "fleet inference provider" pick + one key** (adding Gemini), because the fleet is moving to one key for all daemons. When PRD-008 lands, this step asks for one key rather than presenting a provider list; the what/why explainer and the single `provider === "configured"` completion signal are unchanged. If PRD-013 ships first, it ships the Anthropic-default selector as written and PRD-008c revises it.
