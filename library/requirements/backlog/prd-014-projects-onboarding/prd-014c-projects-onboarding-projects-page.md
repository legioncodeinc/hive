# PRD-014c: The Projects Page and "Add a Project"
...
> **Parent:** [PRD-014](./prd-014-projects-onboarding-index.md)
> **Status:** Backlog
> **Priority:** P1
> **Effort:** L (1-3d)
...
- A top-right **Add a project** button runs the 014b daemon-served folder-pick → bind flow, after which the new project appears in the list.
...
- Cross-device import (014d owns "bind an existing cloud project").
...
| c-AC-3 | Given the page, when the user clicks **Add a project** (top-right), then the 014b folder-pick → bind flow runs and the new project appears on success. |
...
- **Nav entry:** add to the nav shell sidebar ([PRD-037a](../../../../../honeycomb/library/requirements/completed/prd-037-dashboard-nav-shell/prd-037a-dashboard-nav-shell-sidebar.md)); the page is a new dashboard route alongside memories/graph/sync.
...
- **Add/unbind:** the same daemon bind route as 014b; unbind removes the `projects.json` binding for the folder without touching the registry row.
- **Distinguish bound-here vs registry-only:** a project can exist in the registry (made on another device) without a local binding — show those as importable (handoff to 014d), not as actively-sourcing.

## Open questions

- [ ] Live vs on-demand per-project stats (counts, last capture) — render lazily to keep the list fast?
- [ ] Should "Add a project" and "Import project from cloud" (014d) be one combined "+ Add" menu (New folder / Import existing) or two distinct buttons? (Lean: one "+" with two options.)
...
- [PRD-014b: Folder Picker](./prd-014b-projects-onboarding-folder-picker.md) — the Add-a-project flow reused here.
- [PRD-014d: Cross-Device Import](./prd-014d-projects-onboarding-cross-device-import.md) — registry-only projects link into this list.
- [PRD-037a: Dashboard Nav Shell](../../../../../honeycomb/library/requirements/completed/prd-037-dashboard-nav-shell/prd-037a-dashboard-nav-shell-sidebar.md) — where the nav entry slots in.
- [PRD-049e: Dashboard Scope Switcher](../../../../../honeycomb/library/requirements/completed/prd-049-multi-project-and-context-switching/prd-049e-multi-project-and-context-switching-dashboard-scope-switcher.md) — the view scope the per-project "open" drives.
