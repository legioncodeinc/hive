# PRD-014b: First-Run Empty State and the Folder Picker
...
- On bind, the empty state is replaced by the Projects page (014c) and capture begins (014a gate opens).

## Non-Goals

- The Projects management page itself (014c) and cross-device import (014d).
...
| b-AC-4 | Given the user confirms, when the bind completes, then the absolute path → project is written to the 049a store, the 014a gate opens, and the UI advances to the Projects page. |
...
- **Bind:** `POST /api/.../projects/bind { path, name }` → writes via the same code path as `honeycomb project bind` ([`src/cli/project.ts`](../../../../../honeycomb/src/cli/project.ts)) so the dashboard and CLI never diverge on the store format.
- **Name suggestion:** reuse `suggestProjectId` (049d) so the dashboard and CLI suggest identically.
- **Empty-state detection:** the same 014a "zero bound projects" predicate drives whether the dashboard shows the CTA vs the Projects page.
...
- [PRD-014a: Capture Gating](./prd-014a-projects-onboarding-capture-gating.md) — the gate this CTA resolves by binding.
- [PRD-049d](../../../../../honeycomb/library/requirements/completed/prd-049-multi-project-and-context-switching/prd-049d-multi-project-and-context-switching-org-workspace-switching.md) — `bind` store + `suggestProjectId` reused here.
- [PRD-050a: Quick Install](../../../../../honeycomb/library/requirements/completed/prd-050-quick-install-and-guided-setup/prd-050-quick-install-and-guided-setup-index.md) — the guided-setup dashboard this empty state lives inside.
- [`src/dashboard/web/scope-context.tsx`](../../../../../honeycomb/src/dashboard/web/scope-context.tsx) · [`src/cli/project.ts`](../../../../../honeycomb/src/cli/project.ts)
