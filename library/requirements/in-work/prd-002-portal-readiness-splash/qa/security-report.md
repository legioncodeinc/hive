...
**Branch:** `feature/prd-002-portal-readiness-splash` (worktree `the-apiary-thehive-002`), based on `feature/prd-001-thehive-portal-daemon`.
...
- `src/shared/constants.ts` (MODIFIED, `HIVEDOCTOR_STATUS_URL`)
...
One **Medium** finding, remediated in-session: the hivedoctor status fetch used native `fetch` with its default `redirect: "follow"`, so the `isLoopbackBaseUrl()` pin (fs-AC-9) validated only the initial URL. A rogue or compromised loopback service answering on `127.0.0.1:3852` could 3xx-redirect the fetch to a non-loopback origin, silently defeating the loopback pin (SSRF-adjacent, the same threat class PRD-001 hardened against a tampered registry). Fixed by pinning `redirect: "error"` on the fetch so any redirect rejects before the off-loopback request fires; covered by two new tests.

Everything else on the PRD-002 attack surface was reviewed and found clean: the status URL is a hard-pinned loopback constant not derivable from any request/registry/env input; the client-facing response body is limited to the normalized `{ supervisor, health, daemons, asOf }` shape with the fail-soft path leaking nothing about why hivedoctor was unreachable; the hivedoctor body is zod-validated at the boundary with `escalation` held as opaque `unknown`; `ReadinessSplash` genuinely does not mount `SetupGate` (and therefore never calls `/setup/state`) until `isFleetReady()` passes; the built browser bundle contains no Node imports; the route returns JSON only with no request-driven redirect; and the PRD-001 loopback / federated-`wire` posture is unchanged. No Critical or High findings.
...
|---|---|---|---|
| 1 | SSRF / loopback pinning of hivedoctor fetch (fs-AC-2, fs-AC-9) | ATTN -> FIXED | 1 (Medium, remediated) |
...
|---|---|---|---|
| **Status URL is hard-pinned loopback, not derivable** (fs-AC-2) | Fetch target is the `HIVEDOCTOR_STATUS_URL` constant, never from registry / request / query / env | `src/shared/constants.ts:10` (`HIVEDOCTOR_STATUS_URL = "http://127.0.0.1:3852/status.json"`); `src/daemon/server.ts:79` (`options.hivedoctorStatusUrl ?? HIVEDOCTOR_STATUS_URL` - injectable only at daemon construction, not per-request); `fetchFleetStatus` default `url = HIVEDOCTOR_STATUS_URL` | OK |
...
| **zod at the external boundary** (fs-AC-4) | hivedoctor body `safeParse`d before trust; malformed body fail-softs, does not throw into the handler; `escalation` opaque | `HivedoctorStatusSchema` (`fleet-status.ts:29-33`) with `daemons` optional-defaulted; `safeParse` + `if (!parsed.success) return UNREACHABLE_RESPONSE` (`58-61`); `escalation: z.unknown().nullable().optional()` (`26`) | OK |
...
Minimal blast radius: only these two (new/untracked PRD-002) files were edited; no unrelated changes. No new runtime dependency added (zod was already present and is acceptable for the-hive).
