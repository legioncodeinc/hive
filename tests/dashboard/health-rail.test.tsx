...
/**
 * hive PRD-005a — the top health rail. Present on every route (hr-AC-1), shares the
...
describe("HealthRail", () => {
				...
				if (url.includes("/api/fleet-status")) {
					return jsonResponse({
						...
						daemons: [
							...
							{ name: "nectar", health: "degraded", escalation: null },
						],
					});
				}
	...
	it("hr-AC-1/hr-AC-2: renders one pill per service using the shared status vocabulary", async () => {
		...
		expect(screen.getByTestId("health-rail-pill-nectar")).toBeTruthy();
		expect(screen.getByTestId("health-rail-pill-nectar").getAttribute("data-state")).toBe("degraded");
	});

	it("exposes each pill's STATE in its accessible text, not only in `title`/the aria-hidden icon", async () => {
		render(<HealthRail />);
		await waitFor(() => expect(screen.getByTestId("health-rail-pill-nectar")).toBeTruthy());
		// The live region announces text content; a screen reader must hear "nectar: degraded",
		// never a bare "nectar" with the state trapped in a tooltip or a hidden icon.
		expect(screen.getByTestId("health-rail-pill-nectar").textContent).toContain("degraded");
